...
program define pmstabilitysstte, rclass
...
********************************
// Calculate Y as minimum of log event/censoring time
tempvar y w

qui gen `y' = ln(`time')
qui gen `w' = exp(`y')*exp(`lp_var')
...
forvalues col=1/`elements' {
	forvalues row=`row_start'/`elements' {
		...
		qui gen M_`row'`col' = (`varname`row'')*(`varname`col'')*(`w')
		qui su M_`row'`col'
...
tempvar p_true
qui gen `p_true' = 1-(exp((-exp(`lp_var'))*`timepoint'))
...
tempvar lower_lograte_`num'
tempvar upper_lograte_`num'
...
qui gen `lower_lograte_`num'' = `lp_var' - (1.96 * sqrt(`var_ind_`num''))
qui gen `upper_lograte_`num'' = `lp_var' + (1.96 * sqrt(`var_ind_`num''))

qui gen `lower_p_`num'' = 1 - exp((-exp(`lower_lograte_`num''))*5)
qui gen `upper_p_`num'' = 1 - exp((-exp(`upper_lograte_`num''))*5)
